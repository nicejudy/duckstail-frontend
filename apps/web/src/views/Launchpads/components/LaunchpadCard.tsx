...
export function LaunchpadCard({
  ...
  const debouncedQuery2 = useDebounce(data.buyToken, 200)
  const searchToken = useToken(debouncedQuery1)
  const _searchBuyToken = useToken(debouncedQuery2)
  ...
  return (
      ...
      <Box p="20px">
        ...
        {data.presaleType === "standard" && searchToken && searchBuyToken && <Box mb="1rem">
          <Text bold fontSize="26px">{searchToken.name}</Text>
          <Text>1 {searchBuyToken.symbol} = {(data.rate / (10 ** searchToken.decimals)).toLocaleString()} {searchToken.symbol}</Text>
        </Box>}
        {data.presaleType === "fair" && searchToken && searchBuyToken && <Box mb="1rem">
          <Text bold fontSize="26px">{searchToken.name}</Text>
